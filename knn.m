...
for k=1:numel(svmModel)
 ...
 %Train the classifier here knn classifier is implemented
 knn{k}= ClassificationKNN.fit(dataset(idx,:),g(idx),'Distance','seuclidean'); 
% Make a prediction for the tst set
predTest(:,k)=knn{k}.predict(dataset(testIdx,:));
...
% dataset(:, end) = [];
% here we get the predicted output

for k = 1:numel(knn{k})
    %# test
    predQueryImg(:, k) = knn{k}.predict(queryImageFeatureVector); 
